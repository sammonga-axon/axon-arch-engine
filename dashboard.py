...
# --- CSS: MINIMAL & CLEAN (No Button Overrides) ---
st.markdown("""
    <style>
    /* 1. Global Theme */
    ...
    /* 2. Visual Elements */
    .hash-box { background-color: #f8fafc !important; color: #0f172a !important; border: 1px solid #cbd5e1; padding: 15px; border-radius: 6px; font-family: 'JetBrains Mono', monospace; font-size: 14px; margin-top: 5px; }
    .verdict-success { color: #166534 !important; background-color: #dcfce7; padding: 10px; border-radius: 6px; border: 1px solid #bbf7d0; margin-top: 10px; font-weight: 600; }
    .verdict-fail { color: #991b1b !important; background-color: #fee2e2; padding: 10px; border-radius: 6px; border: 1px solid #fecaca; margin-top: 10px; font-weight: 600; }
...
with st.sidebar:
    ...
    # 100% Verified Badge (High Visibility)
...
with c1:
    ...
    st.caption("Immutable Ledger for Vector Embeddings & Model Weights | v3.2.0 (Enterprise)")
...
# --- TAB 1: OVERVIEW (RESTORED LOG TABLE) ---
with tab1:
...
with tab2:
    ...
    # Using Form to fix "Invisible Cursor" and "Black Button"
    with st.form("seal_form"):
        ...
        if submitted:
            if data_to_seal:
                # STRIP Whitespace to ensure Hash Match
                    ...
                    if threat["status"] == "DETECTED":
                        ...
                        try:
                            ...
                            if res.status_code == 200:
                                ...
                                st.markdown(f'<div class="verdict-success">🛡️ SIEM CLEARANCE: GRANTED<br>Deep Packet Inspection Complete.</div>', unsafe_allow_html=True)
...
with tab3:
    ...
    with st.form("audit_form"):
        ...
        if audit_submitted:
            ...
            calculated_hash = hashlib.sha256(clean_data.encode()).hexdigest()
            
            if calculated_hash == clean_root:
                st.balloons()
                st.markdown(f'<div class="verdict-success">✅ VERIFIED: SECURE<br>Mathematical Proof Confirmed. Data is Untainted.</div>', unsafe_allow_html=True)
            else:
                st.markdown(f'<div class="verdict-fail">🚨 ALERT: INTENT INVALIDATION<br>Hash Mismatch Detected.</div>', unsafe_allow_html=True)
                st.write("### Forensic Report:")
                st.code(f"Input:      {clean_data}\nCalculated: {calculated_hash}\nExpected:   {clean_root}", language="text")